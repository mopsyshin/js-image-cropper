...
function readURL(input) {
  if (input.files && input.files[0]) {
    ...
    setTimeout(function(){
      ...
      $('#crop-area-image')[0].style.transform = "translate(" + 0 + "px," + 0 + "px)";
    }, 50);
    // $('#preview').css({'width' : modiWidth, 'height' : modiHeight });
    targetRect = $('#preview')[0].getBoundingClientRect();
    setTimeout(function(){
      modiWidth = $('#preview')[0].getBoundingClientRect().width;
      modiHeight = $('#preview')[0].getBoundingClientRect().height;
      cropAreaRect = $('#crop-area')[0].getBoundingClientRect();
      ...
      if ( cropRatio >= 1) {
        if (modiHeight / cropRatio > modiWidth) {
          $('#crop-area').css({'width': modiWidth, 'height': modiWidth * cropRatio, 'top' : 0, 'left' : 0 });
        } else {
          $('#crop-area').css({'width': modiHeight / cropRatio, 'height': modiHeight, 'top' : 0, 'left' : 0 });
        }
        // console.log('세로형크롭');
      } else {
      ...
      // register resizer
      $('#resize-handler')[0].style.transform = "translate(" + (parseInt($('#crop-area').css('left'), 10) + cropAreaRect.width - 6) + "px," + (parseInt($('#crop-area').css('top'), 10) + cropAreaRect.height - 6) + "px)";
    }, 100);
  }
}
...
function doDrag(e) {
   ...
   cropAreaRect = $('#crop-area')[0].getBoundingClientRect();
   $('#resize-handler')[0].style.transform = "translate(" + (parseInt($('#crop-area').css('left'), 10) + cropAreaRect.width - 3) + "px," + (parseInt($('#crop-area').css('top'), 10) + cropAreaRect.height - 3) + "px)";
}

function stopDrag(e) {
  resizeState = false;
  var condition = $('#crop-area')[0].getBoundingClientRect().right > $('#preview')[0].getBoundingClientRect().right || $('#crop-area')[0].getBoundingClientRect().bottom > $('#preview')[0].getBoundingClientRect().bottom;
  if ( condition ) {
...
function setEventListner() {
  $('#btn-size-cal').click(function(){
    ...
    cropAreaRect = $('#crop-area')[0].getBoundingClientRect();
    targetRect = $('#preview')[0].getBoundingClientRect();
    var modifiedRate = $('#crop-area')[0].getBoundingClientRect().width / (cropWidth / resizeRate),
